...
enum PDFReaderSheet: String, Identifiable {
    case contents, position, search, appearance
...
/// bookmarks) so the chrome can be shared, but renders fixed
...
final class PDFReaderViewModel {
    ...
    /// PDFView, so deduping on `page` would drop every external page turn.
    ...
    /// Maps a whole-document fraction back to a page index.
